...
def active_monkeys(path):
    ...
    worry_management = set()
    for monkey in monkeys:
        worry_management.add(monkey.test)
    worry_management = math.prod(worry_management)

    for round in range(10000):
        for monkey in monkeys:
            while monkey.has_items():
                item = monkey.get_first_item()
                item = monkey.calculate_worry_level(item) % worry_management
